...
**架构图**
```
用户代理                邮件服务器              SMTP              邮件服务器              用户代理
   |                       |                   |                   |                      |
   |                       |                   |                   |                      |
 用户A                  发送方邮件服务器 ------------------> 接收方邮件服务器              用户B
```

**注意点**
* 📋 **术语**：User Agent - 用户代理、Mail Server - 邮件服务器、SMTP - 简单邮件传输协议
...
**注意点**
* 📋 **术语**：HELO - 问候命令、MAIL FROM - 邮件发件人、RCPT TO - 邮件收件人、DATA - 数据传输
...
| **对象封装** | 每个对象封装在各自的响应报文中 | 多个对象包含在一个报文中 |
